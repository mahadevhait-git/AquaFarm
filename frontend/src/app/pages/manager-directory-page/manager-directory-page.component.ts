...
@Component({
  selector: 'app-manager-directory-page',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './manager-directory-page.component.html',
})
export class ManagerDirectoryPageComponent implements OnInit {
  rows: DirectoryUser[] = [];
  loading = true;
  errorMessage = '';
  searchText = '';
  ...
  selectedManager: DirectoryUser | null = null;
  ...
  async toggleManagerPonds(row: DirectoryUser): Promise<void> {
    if (this.selectedManager?.id === row.id) {
  ...
  private async loadManagers(): Promise<void> {
    ...
    this.errorMessage = '';
    try {
      const response = await firstValueFrom(this.apiService.adminUsers.managers());
      this.rows = Array.isArray(response) ? response : [];
    } catch (error) {
      this.errorMessage = this.getErrorMessage(error, 'Failed to load managers.');
      this.rows = [];
    } finally {
      this.loading = false;
    }
